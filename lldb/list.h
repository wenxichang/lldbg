...
#else
extern void __list_add(struct list_head *nodenew,
			      struct list_head *prev,
			      struct list_head *next);
#endif
 ...
 * @param nodenew new entry to be added
 ...
 * @nodenew : the new element to insert
 *
 * If @old was empty, it will be overwritten.
 */
static inline void list_replace(struct list_head *old,
				struct list_head *nodenew)
{
	nodenew->next = old->next;
	nodenew->next->prev = nodenew;
	nodenew->prev = old->prev;
...
#define list_for_each_entry2(pos, head, type, member)			\
 ...
 * @param head the head of the list
 */
static inline unsigned long list_count(const struct list_head *head)
{
...
static inline void hlist_move_list(struct hlist_head *old,
				   struct hlist_head *nodenew)
{
	nodenew->first = old->first;
	if (nodenew->first)
		nodenew->first->pprev = &nodenew->first;
	old->first = NULL;
}
